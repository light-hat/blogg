...
- [Безопасность](#безопасность)
	...
	- [Настройка nginx против DDoS](#настройка-nginx-против-ddos)
	- [Блокировка IP-адресов](#блокировка-ip-адресов)
		- [Блокировка на сайте](#блокировка-на-сайте)
		- [Блокировка на файерволе](#блокировка-на-файерволе)
	- [fail2ban](#fail2ban)
...
Оптимально заливать бэкап на чистую инфраструктуру, когда база данных пуста. Скрипт не идеален, но если вдруг сайт поломают, можно будет восстановиться с помощью этих скриптов.

### Логи

Отслеживать логи пока только вручную. Вот [файл](logs.md), где указано, где какие файлы журналов находятся и за что они отвечают. Чтобы долго не искать, вот команда, чтобы отобразить последние, скажем, 20 записей из лога:

```
tail -n 20 /path/to/log/file
```

В будущем может допишу инструмент, с помощью которого можно будет как-то с логами работать.

### Firewall

#### Установка и настройка

Теперь про то, как установить firewall. Используется `ufw`. Он должен быть предустановлен (в ubuntu по крайней мере точно), но если по какой-то причине его нет, то устанавливается он вот так:
...
Разрешаем подключение `ssh` и разрешаем подключения на порту, где он работает (например, пусть будет 2222 порт):
...
После этого разрешим подключение к нашему серверу по протоколу `http`:
...
Теперь разрешим подключения по `https`:
...
Чтобы посмотреть существующие правила файервола:

```
sudo ufw status verbose
```

Другой вариант команды выше - отобразить правила в нумерованном порядке:

```
sudo ufw status numbered
```

Вот команда, чтобы добавить правило:

```
sudo ufw `тип правила` `протокол или порт`
```

Под типом правила имеется в виду, что это разрашающее правило (обозначается как `allow`) или запрещающее (обозначается как `deny`). Следом указывается порт или название протокола, для которого мы создаём правило.

Чтобы удалить правило, есть два варианта. Первый - это по номеру правила, второй по фактическому названию. Примеры:
...
Чтобы сбросить настройки файервола используется команда:

```
sudo ufw reset
```

Чтобы отключить файервол:

```
sudo ufw disable
```

Ну вот, это была краткая инструкция, как использовать файервол для веб-сервера.

### Настройка nginx против DDoS

Теперь приведу настройки сервера `nginx`, чтобы его было не так просто положить при DDoS атаках.
...
На сайте предусмотрено блокирование IP-адресов, если такая необходимость возникает. В административной панели Django это раздел `security` для модели `Visitors`. Там фиксируются IP-адреса, которые когда-либо посещали сайт. Переходим к адресу, ставим флажок, что он заблокирован. Можно ещё написать причину, это поле ниже. Тогда посетитель с этим IP-адресом сможет увидеть причину своей блокировки.

#### Блокировка на файерволе

Чтобы заблокировать IP-адрес на файерволе используется следующая команда:

```
sudo ufw deny from `зловредный IP-адрес`
```

Второй вариант надёжнее, но зато при первом можно оставить злоумышленнику послание на прощание =)
